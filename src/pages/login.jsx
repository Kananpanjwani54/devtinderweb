...
const Auth = () => {
  ...
  const handleAuth = async () => {
    ...
    try {
      ...
      if (isLoginForm) {
        // Login API
        res = await api.post("/auth/login", { gmail: email, password });
      } else {
        // Signup API
    ...
    <div className="flex justify-center my-16">
      <div className="card bg-base-300 text-base-content w-96 shadow-2xl rounded-xl">
        <div className="card-body space-y-4">
          ...
          {/* Email */}
          <label htmlFor="email" className="input input-bordered flex items-center gap-2 bg-base-200 w-full">
            <h3>Email:</h3>
            <input
              id="email"
              name="email"
          ...
          {/* Password */}
          <label htmlFor="password" className="input input-bordered flex items-center gap-2 bg-base-200 w-full">
            <h3>Password:</h3>
            <input
              id="password"
              name="password"
          ...
          {!isLoginForm && (
            <>
              ...
              <input
                ...
                onChange={(e) => {
                  const value = parseInt(e.target.value, 10);
                  if (value >= 19 && value <= 59) setAge(value);
                  else if (e.target.value === "") setAge("");
                  else setError("Age must be between 19 and 59.");
                }}
              />
            </>
          )}
          ...
          <p className="text-center mt-2">
            {isLoginForm ? "New user?" : "Already have an account?"}{" "}
            <span className="text-blue-500 cursor-pointer" onClick={() => setIsLoginForm(!isLoginForm)}>
              {isLoginForm ? "Signup here" : "Login here"}
            </span>
          </p>
        </div>
